...
begin
	p_sr = plot(x, h, legend = :bottom, label="")
	for sr in unique(SR)
...
begin
	...
	p4 = bar(["trapped", "exited"], [v_trapped, v_exited], title="C02 volume", label="", ylims=(0,2))
	
	plot(p3, p4, size=(900,300))
end

# ╔═╡ 822c9c25-4d3f-4bae-ac29-cb24ccd5aebb
function get_bound(polygon, dim, fn)
	return fn([p[dim] for p in polygon.vrep.points.points])
end

# ╔═╡ 2579caba-b063-41ea-967d-56c6c347e6ad
function depth(polygons, x_obs)
	for p in polygons
		...
		if x_obs >= xmin && x_obs <= xmax
			...
			ystart = get_intersection(pts[[p[2] != ymin for p in pts]]..., vline...)[2]
			return ystart - ymin
		end
	end
	0
...
# ╔═╡ e4e93283-d532-4152-b4d4-3fa49505bbcc
depth(poly, 0.5)
